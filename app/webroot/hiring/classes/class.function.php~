...
class fun{
   ...
   // email validation
	...
	// function to display drive date
	function drive_date($date){
		if(($date != '') && ($date != '0000-00-00')){
			$date_format = date('d', strtotime($date)).'<sup style="vertical-align:super">'.date('S', strtotime($date)).'</sup>'.date('M y', strtotime($date));
			return $date_format;
		}
	}
   
   // function to validate database drive type field 
	public function drive_type($type){
		if($type == '1'){
			$sub = 'On Campus';
		}else{	
	 		$sub = 'Off Campus';
