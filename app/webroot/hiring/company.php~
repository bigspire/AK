...
$query = "CALL list_company('".$keyword."','".$state."','".$district."','".$status."','".$from_date."','".$to_date."','0','0','','','".$_GET['action']."')";
try{
	if(!$result = $mysql->execute_query($query)){
		throw new Exception('Problem in executing list company page');
	}
...
$sort_fields = array('company_name','email','phone','users','district_name','created_date','status');
$org_fields = array('company_name','email','phone','users','district_name','created_date','status');
...
// fetch all records
$query =  "CALL list_company('".$keyword."','".$state."','".$district."','".$status."','".$from_date."','".$to_date."','$start','$limit','".$field."','".$order."','".$_GET['action']."')";
try{
	if(!$result = $mysql->execute_query($query)){
		throw new Exception('Problem in executing list company page');
	}
	...
	if($_GET['action'] == 'export'){ 
      ...
      $excelObj->printHeader($header = array('Company Name','Email','Phone','No. Users','Location','Created') ,$col = array('A','B','C','D','E','F'));  
		// function to print the excel data
		$excelObj->printCell($data, $count,$col = array('A','B','C','D','E','F'), $field = array('company_name','email','phone','no_of_users','district_name','created_date'),'Company_'.$current_date);
	}
	
	// create validation
	if($_GET['current_status'] == 'created' || $_GET['current_status'] == 'updated' || $_GET['current_status'] == 'deleted'){
 	   $success_msg = 'Company ' . $_GET['current_status'] . ' successfully';
...
// status drop down
$smarty->assign('company_status', array('' => 'Status', '1' => 'Active', '2' => 'Inactive'));
...
// assign page title
$smarty->assign('page_title' , 'Search Company - AssessKey');  
